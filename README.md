...
| `QUOTE_THEME`             | 生成语录卡片时默认使用的主题/皮肤名称。                                                                      | `qq-native`                                  |
| `QUOTE_TEXT_ONLY_THEME`   | 仅用于纯文本（可包含@）的单条语录的主题。留空则默认使用 `QUOTE_THEME`。                                      | `""`                                         |
...
-   `语录主题` / `quote theme`：查看所有可用的语录卡片主题（带序号）。
-   `语录主题 [主题名]` / `quote theme [主题名/序号]`：切换当前群聊的默认语录主题（管理员权限）。
...
#### 基础管理 (管理员权限)
-   `quote del` / `语录 删除` (回复语录)：删除被回复的语录及其图片文件。
...
1. **使用 `quote theme` 命令**：查看所有可用主题，每个主题都有对应的序号。
2. **使用主题名称**：例如 `记录 -s classic` 或 `语录主题 classic`。
3. **使用主题序号**：例如 `记录 -s 1` 或 `语录主题 1`（序号更快捷）。
4. **设置默认主题**：管理员可使用 `语录主题 [名称/序号]` 为当前群聊设置默认主题。
5. **纯文本专属主题**：在配置中设置 `QUOTE_TEXT_ONLY_THEME`，为纯文本语录指定专属主题。

> **提示**：主题组件位于 `quote/assets/components/` 目录，开发者可参考现有组件创建自定义主题。
