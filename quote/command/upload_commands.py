...
from ..config import get_author_font_path, get_font_path, quote_path
...
async def save_img_handle(bot: Bot, event: MessageEvent, arp: Arparma, state: T_State):
    ...
    if file_name:
        try:
            ...
            image_path = os.path.join(quote_path, base_img_name)
    ...
    if "group" in session_id:
        ...
        if duplicate:
            ...
            final_image_path = os.path.abspath(os.path.join(quote_path, image_name))
...
tags=aaa bbb ccc"""
    ...
    image_files = copy_images_files(your_path_str, gocq_path_str)
    total_len = len(image_files)
    idx = 0

    for img_bytes, img_rel_path in image_files:
